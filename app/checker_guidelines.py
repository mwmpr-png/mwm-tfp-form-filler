...
def apply_checker_guidelines(data: dict[str, Any]) -> dict[str, Any]:
    ...
    # Preserve the Form Filler's long-standing protection against generic team
    # labels being mistaken for the actual FA Representative's name.
    adviser_name = _text(data, "adviser_name")
    if adviser_name.upper() in {"MWM PR", "MWM ADMIN", "MWM CREATIVE", "ADMIN", "TEST"}:
        data["adviser_name"] = ""
