...
from .checker_guidelines import apply_checker_guidelines
...
def render_first_page_image(path: Path, max_px: int = 1800) -> tuple[str, str] | tuple[None, None]:
    try:
        ...
        if path.suffix.lower() == ".pdf":
            doc = fitz.open(str(path)); page = doc[0]
            pix = page.get_pixmap(matrix=fitz.Matrix(2.5, 2.5), alpha=False)
            im = Image.frombytes("RGB", [pix.width, pix.height], pix.samples); doc.close()
        else:
            im = Image.open(path).convert("RGB")
        im.thumbnail((max_px, max_px)); bio = io.BytesIO(); im.save(bio, format="JPEG", quality=88)
...
def local_ocr_id(path: Path) -> str:
    ...
    try:
        ...
        if path.suffix.lower() == ".pdf":
            ...
            base = Image.frombytes("RGB", [pix.width, pix.height], pix.samples); doc.close()
        else:
...
def openai_vision_extract_id(path: Path) -> dict[str, str]:
    ...
    try:
        ...
        prompt = (
            ...
            "Return only JSON with these keys when visible: client_name, nric, dob, gender, nationality, birthplace, residential_address, postal. "
            "Use exact document text. For dob use DD/MM/YYYY. Do not guess unreadable details."
        )
        resp = client.chat.completions.create(model=model, temperature=0, max_tokens=600, messages=[{"role":"user","content":[{"type":"text","text":prompt},{"type":"image_url","image_url":{"url":f"data:{mime};base64,{b64}"}}]}])
        content = resp.choices[0].message.content or "{}"
        m = re.search(r"\{.*\}", content, flags=re.S)
        obj = json.loads(m.group(0) if m else content)
        out = {k: clean(v) for k, v in obj.items() if clean(v)}
        if out.get("nric"): out["nric"] = normalise_nric_from_text(out["nric"])
        if out.get("dob"): out["dob"] = parse_date_ddmmyyyy(out["dob"])
        if out.get("nationality"): out["nationality"] = normalise_nationality(out["nationality"], out.get("nric", ""))
...
def extract_id_document(path: Path) -> dict[str, str]:
    ...
    out = parse_id_text(text)
    if out.get("nric") and out.get("client_name"):
        return out
    local = parse_id_text(local_ocr_id(path))
    out.update({k: v for k, v in local.items() if v and not out.get(k)})
    if out.get("nric") and out.get("client_name"):
        return out
    vision = openai_vision_extract_id(path)
    out.update({k: v for k, v in vision.items() if v and not out.get(k)})
    return out


def _clean_name_candidate(value: str, client_name: str = "") -> str:
    value = clean(value)
    if not value: return ""
    value = re.sub(r"\s+", " ", value).strip(" -,:;/")
    if len(value) < 3 or len(value) > 90: return ""
    bad = ("financial consultant", "organisation", "date", "illustration", "representative", "prepared for")
    if any(x in value.lower() for x in bad): return ""
    if client_name and value.lower() == clean(client_name).lower(): return ""
    return value


def extract_adviser_name_from_text(txt: str, client_name: str = "") -> str:
    candidates: list[str] = []
    m = re.search(r"Group Insurance\.\s*\n\s*([^\n]{2,90})\s*\n\s*([^\n]{2,90})\s*\n\s*(?:STEP\s*1|Step\s*1)", txt, flags=re.I)
    if m: candidates.append(m.group(2))
    for pat in (r"Your FA Representative\(s\)\s*\n\s*([^\n]{2,90})", r"Financial Consultant'?s name\s*\n\s*([^\n]{2,90})", r"FA Rep.?s Name\s*\n(?:[^\n]*\n){0,8}?\s*([A-Z][A-Z \-().']{3,90})\s*\n\s*(?:[A-Z0-9]{4,12}|\d{1,2}\s+[A-Z]{3})"):
        for m in re.finditer(pat, txt, flags=re.I): candidates.append(m.group(1))
    for m in re.finditer(r"Sighted and verified\s+by\s+([A-Z][A-Za-z \-().']{3,90})", txt, flags=re.I): candidates.append(m.group(1))
...
def extract_hsbc_tfp_personal(text: str) -> dict[str, Any]:
    ...
    if m:
        out["client_name"] = clean(m.group(1)).title(); out["nric"] = m.group(2).upper(); out["nationality"] = normalise_nationality(m.group(3), out["nric"]); out["dob"] = parse_date_ddmmyyyy(m.group(4)); out["birthplace"] = clean(m.group(5)).title()
    if not out.get("client_name"):
        m = re.search(r"Last Name/Surname\s*\n\s*First/Given Name[\s\S]{0,120}?\n\s*([A-Z][A-Z'-]+)\s*\n\s*([A-Z][A-Z\s'-]+)\s*\n", text, flags=re.I)
        if m: out["client_name"] = clean(m.group(1) + " " + m.group(2)).title()
    if not out.get("nric"):
        nric = normalise_nric_from_text(text)
        if nric: out["nric"] = nric
    if not out.get("dob"):
        dob = find(r"Date of birth\s*\(dd/mm/yyyy\)\s*\n\s*([0-9]{1,2}/[0-9]{1,2}/[0-9]{4})", text)
        if dob: out["dob"] = parse_date_ddmmyyyy(dob)
    if not out.get("residential_address"):
        m = re.search(r"Residential Address[\s\S]{0,420}?\n\s*([A-Z0-9 #,.'/-]+SINGAPORE)\s*\n\s*Postal Code\s*\n\s*(\d{6})", text, flags=re.I)
        if m: out["residential_address"] = clean(m.group(1)).upper(); out["postal"] = m.group(2)
    ...
    if re.search(r"\bFemale\b|IZl Female|\[Z\]\s*Female|Female\s*\n", text, flags=re.I): out.setdefault("gender", "Female")
    elif re.search(r"\bMale\b", text, flags=re.I): out.setdefault("gender", "Male")
    if re.search(r"\bMarried\b", text, flags=re.I): out.setdefault("marital_status", "Married")
    plan = find(r"I recommend the\s+([^\n]+)", text) or find(r"Product\s*Name\s*[:\n ]+([^\n]+)", text)
    if plan: out["plan_name"] = clean(plan)
    if re.search(r"HSBC Life Indexed Flexi Income", text, re.I): out.setdefault("plan_name", "HSBC Life Indexed Flexi Income")
    if re.search(r"Diamond Prestige|IUL", text, re.I): out.setdefault("plan_name", "HSBC Life Diamond Prestige IUL II")
    ...
    if prem: out["premium"] = prem; out.setdefault("currency", "USD" if "US$" in text or "USD" in text else "SGD")
    fa = find(r"Financial Consultant's name\s*\n\s*([^\n]+)", text); code = find(r"Financial Consultant's code\s*\n\s*([0-9A-Za-z]+)", text)
    if fa and not re.search(r"Financial Consultant|Organisation", fa, re.I): out["adviser_name_from_hsbc"] = clean(fa)
    if code: out["fa_source_code"] = code
    return {k: clean(v) for k, v in out.items() if clean(v)}
...
def extract_from_bi(text: str, fields: dict[str, str]) -> dict[str, Any]:
    ...
    data["premium"] = clean(find(r"(?:Total Premium|Basic Premium)\s*:\s*\$?([0-9,]+(?:\.\d{2})?)", text) or first_field(fields, "undefined_19", "premiums_1", "Premium", "fill_23"))
    data["premium_frequency"] = clean(find(r"Premium Frequency\s*:\s*([^\n]+)", text)); data["currency"] = clean(find(r"Currency\s*:\s*([A-Z]{3})", text)) or "SGD"; data["mip"] = clean(find(r"Minimum Investment Period\s*:\s*([^\n]+)", text)); data["benefit_type"] = clean(find(r"Benefit Type\s*:\s*([^\n]+)", text)); data["age_last_birthday"] = clean(find(r"Age Last Birthday\s*:\s*(\d+)", text))
    data["policy_term"] = clean(find(r"Manulife InvestReady[^\n]*\s+(Up to age 99)\s+(\d+)\s+", text)); data["premium_term"] = clean(find(r"Manulife InvestReady[^\n]*Up to age 99\s+(\d+)\s+", text)); data["sum_assured"] = first_field(fields, "sum_assured", "Sum Assured", "fill_22_2")
    fa_match = re.search(r"\nDate\s*\n([A-Z][A-Z \n().,'/-]{3,140}?)\nDate\s*\nThis illustration", text, flags=re.S)
    if fa_match:
        fa = clean(" ".join(line.strip() for line in fa_match.group(1).splitlines() if line.strip())); fa = _clean_name_candidate(fa, data.get("client_name", ""))
        if fa: data["adviser_name_from_bi"] = fa
    if not data.get("adviser_name_from_bi"):
        fa = extract_adviser_name_from_text(text, data.get("client_name", ""))
        if fa: data["adviser_name_from_bi"] = fa
    if re.search(r"HSBC Life Indexed Flexi Income", text, re.I) and not data.get("plan_name"): data["plan_name"] = "HSBC Life Indexed Flexi Income"
    if re.search(r"HSBC Life Diamond Prestige|Diamond Prestige IUL|IUL II", text, re.I) and not data.get("plan_name"): data["plan_name"] = "HSBC Life Diamond Prestige IUL II"
    if re.search(r"FWD|Invest Flexi Elite", text, re.I) and not data.get("plan_name"): data["plan_name"] = "FWD Invest Flexi Elite"
...
def extract_client(text: str, fields: dict[str, str], adviser_email: str = "") -> dict[str, Any]:
    ...
    if not data.get("dob") and fields.get("Text11") and fields.get("Text12") and fields.get("Text13"): data["dob"] = f"{fields.get('Text11')}/{fields.get('Text12')}/{fields.get('Text13')}"
    ...
    data["marital_status"] = "Widowed" if fields.get("Widowed") or fields.get("checkbox_widowed") else "Married" if fields.get("Married") or fields.get("checkbox_married") else "Single" if fields.get("Single") or fields.get("checkbox_single") else data.get("marital_status", "")
    ...
    if not data.get("postal") and data.get("residential_address"): data["postal"] = find(r"SINGAPORE\s*(\d{6})", data.get("residential_address", ""))
    data["mobile"] = first_field(fields, "Mobile No", "Mobile Number", "mobile_number", "undefined_7"); data["client_email"] = first_field(fields, "Email Address", "email", "undefined_8"); data["occupation"] = first_field(fields, "Occupation", "occupation", "undefined_9"); data["employer"] = first_field(fields, "Employer", "undefined_10"); data["annual_income"] = first_field(fields, "Current Year S", "Annual Earned Income S", "annual_income", "Text119")
    ...
    if direct_source: funding_sources.append(direct_source)
    if first_field(fields, "Employment  Trade Income", "Employment Trade Income"): funding_sources.append("Employment / Trade Income")
    if first_field(fields, "Investment Income"): funding_sources.append("Investment Income")
    if first_field(fields, "Savings"): funding_sources.append("Savings")
    other_source = first_field(fields, "undefined_72")
    if other_source: funding_sources.append(other_source)
    if funding_sources: data["source_of_funds"] = " & ".join(dict.fromkeys(funding_sources))
    data["education"] = first_field(fields, "highest_education_level", "Highest Education Level"); data["english"] = "Yes" if fields.get("checkbox_english_yes") or fields.get("yes1") else ""
    if not data.get("age_next"):
        if data.get("age_last_birthday"):
            try: data["age_next"] = str(int(data["age_last_birthday"]) + 1)
            except Exception: pass
        if not data.get("age_next") and data.get("dob"): data["age_next"] = age_next_birthday(data["dob"])
    data["adviser_email"] = adviser_email
    adviser_from_fields = _clean_name_candidate(first_field(fields, "FAname", "fa_name", "Representatives Name 1", "namefa"), data.get("client_name", "")); adviser_from_text = extract_adviser_name_from_text(text, data.get("client_name", ""))
    ...
    data["fund_code"] = first_field(fields, "Fund CodeRow1"); data["fund_name"] = first_field(fields, "Fund NameRow1", "fundmanager1", "Name of Fund Manager  Investment Product1"); data["fund_allocation"] = first_field(fields, "Text115", "investamount1", "RSP 1"); data["asset_class"] = first_field(fields, "Asset Class"); data["risk_profile"] = first_field(fields, "S  L  JO", "5 Clients Risk Profile")
    oa = find(r"Ordinary Account \(OA\)\s*\$?([0-9,]+\.\d{2})", text); ma = find(r"MediSave Account \(MA\)\s*\$?([0-9,]+\.\d{2})", text); ra = find(r"Retirement Account \(RA\)\s*\$?([0-9,]+\.\d{2})", text)
    if oa or ma or ra: data["cpf_oa"] = oa; data["cpf_ma"] = ma; data["cpf_ra"] = ra; data["cpf_total"] = fmt_money(money_number(oa) + money_number(ma) + money_number(ra))
...
def retirement_calc(expected_yearly: str, cpf_life_yearly: str = "", years: str | int | None = None) -> dict[str, str]:
    exp = money_number(expected_yearly); cpf = money_number(cpf_life_yearly) if cpf_life_yearly else 0.0; shortfall = max(exp - cpf, 0.0)
    try: years_n = int(float(years)) if years not in (None, "") else 0
    except Exception: years_n = 0
    total = shortfall * years_n if years_n > 0 else 0.0
    return {"retirement_amt": fmt_money(exp, compact=True) if exp else "", "retirement_income": fmt_money(cpf, compact=True) if cpf else "", "retirement_shortfall": fmt_money(shortfall, compact=True) if exp else "", "retirement_years": str(years_n) if years_n else "", "retirement_amt_plan": fmt_money(total, compact=True) if total else "", "retirement_total_amt": fmt_money(total, compact=True) if total else ""}


def recommendation_text(data: dict[str, Any], product_type: str, expected_retirement_income: str) -> str:
    working = dict(data); working["expected_retirement_income"] = expected_retirement_income; enrich_case(working, product_type, source_text="", fields={}); apply_checker_guidelines(working); return clean(working.get("recommendation_text"))
...
def build_case(paths: list[Path], adviser_email: str, product_type: str, expected_retirement_income: str = "") -> dict[str, Any]:
    ...
    for key, val in id_data.items():
        if not val: continue
        if key in {"nric", "dob", "gender", "nationality", "birthplace", "residential_address", "postal"}: data[key] = val
        elif not data.get(key): data[key] = val
    if data.get("dob") and not data.get("age_next"): data["age_next"] = age_next_birthday(data["dob"])
    if data.get("residential_address"):
        addr, pc = normalise_address_and_postal(data.get("residential_address"), data.get("postal")); data["residential_address"] = addr
        if pc: data["postal"] = pc
    data["product_type"] = product_type; data["expected_retirement_income"] = expected_retirement_income
    ...
    data = apply_checker_guidelines(data)
